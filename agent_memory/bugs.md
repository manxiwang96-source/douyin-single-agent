...
## Open
- Classmate C input variable names, App IDs, and Dify version are unknown; the real v1 contract may be Dify, CLI, or HTTP. Workflow inputs stay JSONB until the contract arrives.
...
## Closed
- Discussion-only phase is closed: the Douyin redesign is a locked modify doc, including the client entry and `agent_instances`.
