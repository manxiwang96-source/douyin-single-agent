...
- ui/view_model.py: maps media URLs to image/video widgets; HITL card vs chat history; plaza cards; readonly sidebar; auth_gate so a thread exists only after login and /open
- ui/streamlit_app.py: Streamlit v1 client — login/register, plaza cards, open conversation, chat + readonly sidebar; Bearer token in session_state; no unauthenticated POST /v1/threads
...
阶段 0 landed docs/config. 阶段 1 landed business SQL + in-memory/Postgres repositories and production pgvector/schema startup. 阶段 2 landed FastAPI login/register/logout/me and plaza HTTP (create/list/patch/open/sidebar/avatar) with Bearer sessions; creating a douyin_ops instance binds douyin-lead-discovery and seeds demo KB. 阶段 3 landed per-user/instance isolation for ainvoke configurable, profile/KB namespaces, media paths, media_assets, and thread ownership. 阶段 4 landed `discover_douyin_leads` inside the existing ToolNode via DifyClient; default pytest injects FakeDifyClient and `DIFY_LIVE_ENABLED` stays false. 阶段 5 landed `list_jobs` / `cancel_job` in the same ToolNode plus `POST /v1/jobs/{job_id}/disable` and `POST /v1/job-runs/{run_id}/cancel`; disabled definitions refuse new runs; cancel is a status and does not withdraw sent rows; Douyin 08:00 cron is still not implemented. 阶段 6 landed Streamlit login/plaza/readonly sidebar over existing Bearer HTTP; thread is created by POST /open after login, not by unauthenticated POST /v1/threads. Graph nodes remain chatbot + tools. Do not rewrite this file as if DifyClient has replaced the 个人超级助理 graph. `DIFY_LIVE_ENABLED` stays false until phase 7.
