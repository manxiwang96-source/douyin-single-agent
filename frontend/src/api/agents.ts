...
import { createInstancePayload, patchInstancePayload } from "../lib/plaza";
...
export async function patchAgentInstance(
  ...
  fields: { title: string; intro: string; avatar?: string | null },
) {
  const response = await http.patch(
    `/v1/agent-instances/${agentInstanceId}`,
    patchInstancePayload(fields.title, fields.intro, fields.avatar),
  );
  return response.data;
}

export async function archiveAgentInstance(agentInstanceId: string) {
  const response = await http.delete(`/v1/agent-instances/${agentInstanceId}`);
  return response.data;
}
