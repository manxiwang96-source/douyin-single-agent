...
阶段 0 landed docs/config. 阶段 1 landed business SQL + in-memory/Postgres repositories and production pgvector/schema startup. 阶段 2 landed FastAPI login/register/logout/me and plaza HTTP (create/list/patch/open/sidebar/avatar) with Bearer sessions; creating a douyin_ops instance binds douyin-lead-discovery and seeds demo KB. 阶段 3 landed per-user/instance isolation for ainvoke configurable, profile/KB namespaces, media paths, media_assets, and thread ownership. Do not rewrite this file as if DifyClient has replaced the 个人超级助理 graph; that is a later phase. Streamlit is still the old unauthenticated chat entry.
